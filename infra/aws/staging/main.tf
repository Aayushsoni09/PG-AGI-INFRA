...
module "compute" {
  ...
  # staging: slightly larger than dev, mirrors prod topology
  frontend_cpu    = 256
  frontend_memory = 512
  backend_cpu     = 512
  backend_memory  = 1024

  # staging: 1 task minimum, autoscaling on
  frontend_desired_count = 1
  backend_desired_count  = 1
  backend_max_count      = 2
  enable_autoscaling     = true

  # staging: zero downtime deploys (same as prod)
  deployment_min_healthy_percent = 100
  deployment_max_percent         = 200

  # staging: container insights on, longer log retention
